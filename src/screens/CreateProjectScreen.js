...
import { createProject } from '../database/projectsDao';
...
export default function CreateProjectScreen({ navigation }) {
  ...
  const pickCoverImage = async () => {
    ...
    if (status !== 'granted') {
      Alert.alert('Permission requise', "Autorise l'accès à ta galerie pour choisir une couverture.");
      return;
    }
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
          ...
          <View style={styles.header}>
            ...
            <Text style={styles.subtitle}>Raconte-nous ton prochain costume !</Text>
          </View>
          ...
          <View style={styles.form}>
            ...
            {/* Deadline avec DatePicker natif */}
          ...
          <TouchableOpacity
            ...
            disabled={saving}
          >
            <Text style={styles.btnPrimaryText}>{saving ? 'Création...' : 'Créer le projet'}</Text>
          </TouchableOpacity>
